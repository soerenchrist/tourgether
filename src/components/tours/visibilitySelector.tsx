...
const VisibilitySelector: React.FC<{
  ...
  const getInfoText = (visibility: Visibility) => {
    if (visibility === "FRIENDS")
      return "Only you and your friends can see this tour";
    ...
    <div>
      <div className="flex">
        ...
        <Tooltip content={infoText} placement="right">
          <Icon
            path={mdiInformation}
            className="ml-2 text-gray-700 w-5 h-5"
          ></Icon>
        </Tooltip>
      </div>
      <Dropdown id="visibility" label={text}>
        <Dropdown.Item onClick={() => onChange("PUBLIC")}>
          <span className={visibility === "PUBLIC" ? selectedClass : ""}>
            {getText("PUBLIC")}
          </span>
        </Dropdown.Item>
        <Dropdown.Item onClick={() => onChange("FRIENDS")}>
          <span className={visibility === "FRIENDS" ? selectedClass : ""}>
            {getText("FRIENDS")}
          </span>
        </Dropdown.Item>
        <Dropdown.Item onClick={() => onChange("PRIVATE")}>
          <span className={visibility === "PRIVATE" ? selectedClass : ""}>
            {getText("PRIVATE")}
          </span>
        </Dropdown.Item>
      </Dropdown>
